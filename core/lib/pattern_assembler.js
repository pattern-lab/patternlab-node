...
const pattern_assembler = function () {
  ...
  function loadPatternIterative(sourcePath, relPath, patternlab) {

    var relativeDepth = (relPath.match(/\w(?=\\)|\w(?=\/)/g) || []).length;
    if (relativeDepth > 2) {
      console.log('');
      plutils.warning('Warning:');
      plutils.warning('A pattern file: ' + relPath + ' was found greater than 2 levels deep from ' + sourcePath + '.');
  ...
  function processPatternIterative(sourcePath, pattern, patternlab) {
  ...
  return {
    ...
    load_pattern_iterative: function (sourcePath, file, patternlab) {
      return loadPatternIterative(sourcePath, file, patternlab);
    },
