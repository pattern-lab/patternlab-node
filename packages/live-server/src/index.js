...
function staticServer(root) {
  ...
  return function (req, res, next) {
    ...
    const injectCandidates = [new RegExp('</body>', 'i'), new RegExp('</head>', 'i')];
    ...
    function inject(stream) {
      if (injectTag) {
        ...
        const len = INJECTED_CODE.length * injectCount + res.getHeader('Content-Length');
        res.setHeader('Content-Length', len);

        const originalPipe = stream.pipe;
        stream.pipe = function (resp) {
          originalPipe
            .call(stream, es.replace(new RegExp(injectTag, 'i'), INJECTED_CODE + injectTag))
            .pipe(resp);
        };
      }
    }
...
LiveServer.start = function (options) {
  ...
  const watchPaths = options.watch || (options.assets ? [root, ...options.assets] : [root]);
  ...
  server.addListener('error', function (e) {
    if (e.code === 'EADDRINUSE') {
      console.log('%s is already in use. Trying another port.'.yellow, `${protocol}://${host}:${port}`);
      setTimeout(function () {
    ...
    if (LiveServer.logLevel > 2 && address.address === '0.0.0.0') {
      ...
      serveURLs = Object.keys(ifaces)
        ...
        .reduce((data, addresses) => {
          addresses.filter((addr) => addr.family === 'IPv4').forEach((addr) => data.push(addr));
          return data;
        }, [])
