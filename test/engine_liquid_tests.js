...
tap.test('media object liquid pattern can see the atoms-button and atoms-image partials and renders them', function (test) {
  test.plan(1);

  // pattern paths
  var buttonPatternPath = path.join('00-atoms', '00-general', '08-button.liquid');
  var imagePatternPath = path.join('00-atoms', '00-general', '09-image.liquid');
  var mediaObjectPatternPath = path.join('00-molecules', '00-general', '00-media-object.liquid');
...
tap.test('liquid partials can render JSON values', {skip: true}, function (test) {
  test.plan(1);
...
tap.test('liquid partials use the JSON environment from the calling pattern and can accept passed parameters', {skip: true}, function (test) {
  test.plan(1);
...
tap.test('find_pattern_partials finds partials with liquid parameters', function (test) {
  testFindPartials(test, [
